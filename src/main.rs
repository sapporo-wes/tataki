...
#[clap(
  name = "Tataki",
  about = "Tataki is a command line tool for detecting life science data types.",
  version = "0.1.0"
)]
struct Opts {
...
fn is_bed(path: &PathBuf) -> Result<Option<usize>> {
  ...
  for line in contents.lines() {
    if re_comment_header.is_match(line) || re_blank.is_match(line) || re_whitespace.is_match(line) {
    } else {
      match index_re {
        Some(index) => {
